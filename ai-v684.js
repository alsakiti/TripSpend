(() => {
  ...
  function openAi(){const m=$("tripAiModal");if(!m)return;m.classList.remove("hidden");if(!$("tripAiMessages").children.length){addMessage("system",workerActionsReady?"Ask about your trip, request an analysis, or tell me what to change. Nothing is saved without confirmation.":"Enhanced local analysis is ready. Cloud AI is connecting for natural-language requests.");for(const h of chatHistory.slice(-6))addMessage(h.role,h.content);}setTimeout(()=>$("tripAiInput")?.focus(),80);}
  function closeAi(){requestController?.abort();requestController=null;$("tripAiModal")?.classList.add("hidden");}
  function updateServiceStatus(){const b=$("tripAiServiceStatus"),v=$("tripAiWorkerVersion");if(b){b.textContent=endpoint?(workerActionsReady?"AI READY":"ANALYSIS READY"):"LOCAL ONLY";b.classList.toggle("ready",!!endpoint);}if(v)v.textContent=workerVersion?`Worker v${workerVersion}`:"Enhanced client v6.8.4";}
