(() => {
  ...
  function persistUndo(value) { try { if(value)sessionStorage.setItem(UNDO_KEY,JSON.stringify({tripId:state()?.trip?.id||"",before:value.before,after:value.after}));else sessionStorage.removeItem(UNDO_KEY); } catch {} }
  function loadUndo() { try { const value=JSON.parse(sessionStorage.getItem(UNDO_KEY)||"null");return value?.tripId===(state()?.trip?.id||"")&&value?.before&&typeof value.before==="object"&&value?.after&&typeof value.after==="object"?{before:value.before,after:value.after}:null; } catch { return null; } }
  function finishAiAction(before,message) { lastAiUndo={before,after:clone(state())};persistUndo(lastAiUndo);core.save({immediate:true});core.render();return{ok:true,message}; }
