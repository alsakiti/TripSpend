...
test("Arabic Home has localized welcome text and no floating add overlap", async ({ page }) => {
  ...
  await expect(page.locator("#healthTitle")).not.toContainText(/No spending recorded yet|Today’s spending guide|On track|Over budget/);
  await expect(page.locator("#healthText")).not.toContainText(/You have|You can spend up to|planned for this trip|under budget|over budget/);
  ...
  const dynamicArabic = await page.evaluate(() => [
    ...
    "You can spend up to 24 OMR more today and stay on your current plan.",
...
test("Arabic Add Expense sheet fully localizes static and dynamic copy", async ({ page }) => {
  ...
  // Receipt controls are intentionally part of the optional details in v7.2.1.
