...
for (const marker of ["homeGuideCard","countryBudgetToggle","expense-fast-fields","local-first-note"]) {
  if (!index.includes(marker)) fail(`v7.2.5 UX structure missing: ${marker}`);
}
...
const fx = read("fx.js");
for (const marker of ["const APP_RELEASE = \"7.2.5\"","fxCardVisible","settingsAdvanced > summary"]) {
...
for (const phrase of ["Make today easier","TODAY'S GUIDANCE","Your trip and receipts stay on this device unless you export them."]) {
  if (!locale.includes(phrase)) fail(`v7.2.5 Arabic localization missing: ${phrase}`);
}
...
for (const marker of ["TripSpend v7.2.5","home-guide-card","home-guidance-card","A compact route capsule keeps four countries visible without crowding","expense-fast-fields","prefers-reduced-motion:reduce"]) {
  if (!style.includes(marker)) fail(`v7.2.5 visual polish missing: ${marker}`);
}
...
if (!style.includes("html::-webkit-scrollbar") || !style.includes("body::-webkit-scrollbar") || !style.includes("scrollbar-width:none") || !style.includes("-ms-overflow-style:none")) fail("visible scroll indicators are not suppressed");
else ok("scroll indicators are hidden without disabling scrolling");
...
for (const marker of ["setup-history-toggle","setup-history-count","setup-history-content","max-height:38dvh"]) {
  if (!index.includes(marker) && !setupOnboarding.includes(marker)) fail(`compact past trips control missing: ${marker}`);
}
...
if (receipt.includes('addEventListener("submit"') || !receipt.includes('tripspend:expense-saved') || !receipt.includes('addEventListener("reset",clearLearningBaseline)')) fail("receipt learning is not gated by successful save and form lifecycle");
else ok("receipt learning waits for successful save and clears abandoned baselines");
...
if (!intelligence.includes("questionLanguage") || !intelligence.includes("answerText") || !intelligence.includes("explanation(undefined,language)")) fail("local AI answers do not use the question language");
else ok("local AI answers use the question language independently of the UI locale");

const app = read("app.js");
for (const marker of ["← Back to TripSpend","function returnToTripSpend()","window.history.back()","window.location.replace(\"./\")","report-actions no-print"]) {
  if (!app.includes(marker)) fail(`trip report return control missing: ${marker}`);
}
const aiClient = read("ai-v684.js");
if (!app.includes('new CustomEvent("tripspend:expense-saved"')) fail("successful expense saves do not announce receipt-learning completion");
else ok("successful expense saves announce receipt-learning completion");
if (!aiClient.includes("lastAiUndo") || !aiClient.includes("sameState(s,lastAiUndo.after)") || aiClient.includes("lastAiSnapshot")) fail("AI undo can still overwrite intervening manual edits");
else ok("AI undo preserves intervening manual edits");
...
if (!index.includes("appDialogModal") || !index.includes("routeCountryModal") || !index.includes("enhancements-v710.js?v=7.2.5") || !enhancements.includes('"ui-foundation-v710.js"')) fail("v7.2 dialogs or route details are not wired in source");
