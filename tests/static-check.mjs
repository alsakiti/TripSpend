...
for (const marker of ["homeGuideCard","countryBudgetToggle","expense-fast-fields","local-first-note"]) {
  if (!index.includes(marker)) fail(`v7.2.6 UX structure missing: ${marker}`);
}
...
const fx = read("fx.js");
for (const marker of ["const APP_RELEASE = \"7.2.6\"","fxCardVisible","settingsAdvanced > summary"]) {
...
for (const phrase of ["Make today easier","TODAY'S GUIDANCE","Your trip and receipts stay on this device unless you export them."]) {
  if (!locale.includes(phrase)) fail(`v7.2.6 Arabic localization missing: ${phrase}`);
}
...
for (const marker of ["TripSpend v7.2.6","home-guide-card","home-guidance-card","A compact route capsule keeps four countries visible without crowding","expense-fast-fields","prefers-reduced-motion:reduce"]) {
  if (!style.includes(marker)) fail(`v7.2.6 visual polish missing: ${marker}`);
}
...
if (!index.includes("appDialogModal") || !index.includes("routeCountryModal") || !index.includes("enhancements-v710.js?v=7.2.6") || !enhancements.includes('"ui-foundation-v710.js"')) fail("v7.2 dialogs or route details are not wired in source");
