...
test("first visit runs the current source without a service-worker reload", () => {
  ...
  expect(app).toContain('const APP_VERSION = "7.2.6"');
  expect(index).toContain('enhancements-v710.js?v=7.2.6');
...
test("v7.2.6 cache revision serves source files stale-while-revalidate", async ({ page }) => {
  await bootControlled(page);
