...
test("v7.0.10 defers non-visible UI work and serves optimized runtimes", async ({ page }) => {
  await boot(page);
  ...
  expect(runtimes["app.js"]).toContain('const APP_VERSION = "7.0.10"');
  ...
  expect(runtimes["visual-polish-v704.js"]).toContain('const RELEASE = "7.0.10"');
